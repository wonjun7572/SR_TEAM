...
_int CDashCube::Update_Object(const _float & fTimeDelta)
{
	for (list<ATTRIBUTE>::iterator iter = m_particles.begin(); iter != m_particles.end(); iter++)
	{
		...
		iter->vPos += (iter->vVelocity) * fTimeDelta * 2.f;
...
void CDashCube::resetParticle(ATTRIBUTE * attribute)
{
	...
	attribute->vPos = m_vCubePatriclePos + attribute->vVelocity / 1.f;
	attribute->vPos += m_vDir*1.8f;	
	attribute->vPos.y += attribute->vVelocity.y / 1.f;
