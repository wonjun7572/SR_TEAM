...
HRESULT CMob::Ready_Object(const _vec3 & vPos, _tchar * Name)
{
	m_tAbility = new MONSTERABILITY;
	m_tAbility->iLevel = 0;
	m_tAbility->fMaxHp = 200.f;
	m_tAbility->fCurrentHp = m_tAbility->fMaxHp;
	m_tAbility->fDamage = 30.f;
	m_tAbility->strObjTag = L"Mob";
	
	m_MonsterName = Name;

	m_STATE = MOB_IDLE;
	m_BeforeState = MOB_IDLE;
	m_WALK = MOB_WALKSTART;
	m_ATTACK = MOBATTACK_1;
	m_IDLE = MOBIDLE_1;
...
_int CMob::Update_Object(const _float & fTimeDelta)
{
	if (m_bDead)
	{
	...
	if (m_bFirst)
	{
		...
		Load_Animation(L"../../Data/Mob/ALIEN20_WALKINGSTART.dat", 0);
		Load_Animation(L"../../Data/Mob/ALIEN20_WALKING1.dat", 1);
		Load_Animation(L"../../Data/Mob/ALIEN20_WALKING2.dat", 2);
		Load_Animation(L"../../Data/Mob/ALIEN20_WALKING3.dat", 3);
		//Load_Animation(L"../../Data/Mob/ALIEN13_IDLESTART.dat", 0);
		//Load_Animation(L"../../Data/Mob/ALIEN13_IDLE1.dat", 1);
		//Load_Animation(L"../../Data/Mob/ALIEN13_IDLE2.dat", 2);

		//Load_Animation(L"../../Data/Mob/ALIEN13_WALKINGSTART.dat", 3);
		//Load_Animation(L"../../Data/Mob/ALIEN13_WALKING1.dat", 4);
		//Load_Animation(L"../../Data/Mob/ALIEN13_WALKING2.dat", 5);
		//Load_Animation(L"../../Data/Mob/ALIEN13_WALKING3.dat", 6);


		//Load_Animation(L"../../Data/Mob/ALIEN13_ATTACKINGSTART.dat", 7);
		//Load_Animation(L"../../Data/Mob/ALIEN13_ATTACKING1.dat", 8);
		//Load_Animation(L"../../Data/Mob/ALIEN13_ATTACKING2.dat", 9);
	}
...
void CMob::LateUpdate_Object(void)
{
	if (!m_bFirst)
	{
		if (m_STATE == MOB_WALK)
...
HRESULT CMob::Add_Component(void)
{
...
void CMob::Walk_Animation_Run(void)
{
	list<pair<const _tchar*, CGameObject*>> ListBox = *(pMyLayer->Get_GamePairPtr());
	...
	if (m_AnimationTime >= 1.f)
	{
		...
		if (m_WALK == MOB_WALKSTART)
			m_WALK = MOBWALK_1;
		else if (m_WALK == MOBWALK_1)
			m_WALK = MOBWALK_2;
		else if (m_WALK == MOBWALK_2)
			m_WALK = MOBWALK_3;
		else if (m_WALK == MOBWALK_3)
			m_WALK = MOB_WALKSTART;
	...
	{
		CQuarternion* Qtan = dynamic_cast<CQuarternion*>(iter.second->Get_Component(L"Proto_QuaternionCom", ID_STATIC));
		Qtan->Change_Animation(0 + m_WALK);
	}


}

void CMob::Idle_Animation_Run(void)
{
	list<pair<const _tchar*, CGameObject*>> ListBox = *(pMyLayer->Get_GamePairPtr());
	...
	if (m_AnimationTime >= 1.f)
	{
		...
		if (m_IDLE == MOBIDLE_1)
			m_IDLE = MOBIDLE_2;
		else if (m_IDLE == MOBIDLE_2)
			m_IDLE = MOBIDLE_3;
		else if (m_IDLE == MOBIDLE_3)
			m_IDLE = MOBIDLE_1;
...
void CMob::Attack_Animation_Run(void)
{
	list<pair<const _tchar*, CGameObject*>> ListBox = *(pMyLayer->Get_GamePairPtr());
	...
	if (m_AnimationTime >= 1.f)
	{
		...
		if (m_ATTACK == MOBATTACK_1)
			m_ATTACK = MOBATTACK_2;
		else if (m_ATTACK == MOBATTACK_2)
			m_ATTACK = MOBATTACK_3;
		else if (m_ATTACK == MOBATTACK_3)
			m_ATTACK = MOBATTACK_1;
		m_AnimationTime = 0;
	}
	for (auto& iter : ListBox)	// 애니메이션 변경
...
HRESULT CMob::Build(void)
{
	HANDLE      hFile = CreateFile(L"../../Data/Mob/ALIEN20.dat",      // 파일의 경로와 이름	
...
void CMob::Load_Animation(wstring FileName, _uint AnimationID)
{
...
void CMob::Run_Animation(const _float & AnimationSpeed)
{
	list<pair<const _tchar*, CGameObject*>> ListBox = *(pMyLayer->Get_GamePairPtr());
...
CMob * CMob::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3 & vPos, _tchar * Name)
{
	CMob* pInstance = new CMob(pGraphicDev);
