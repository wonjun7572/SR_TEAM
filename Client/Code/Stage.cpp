...
HRESULT CStage::Ready_Layer_Monster(const _tchar * pLayerTag)
{
	...
	if (!vecFireMan.empty())
	{
		for (int i = 0; i < vecFireMan.size(); i++)
		{
			_tchar* szName = new _tchar[256]{};
	...
	pGameObject = CAlien::Create(m_pGraphicDev, _vec3(10.f, 0.6f, 10.f), L"Alien");
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameList(pGameObject), E_FAIL);
...
HRESULT CStage::Ready_Layer_GunItem(const _tchar * pLayerTag)
{
	...
	if (!vecGun.empty())
	{
		pGameObject = CGetShotgun::Create(m_pGraphicDev, vecGun[0]);
		NULL_CHECK_RETURN(pGameObject, E_FAIL);
		...
		pGameObject = CGetSniper::Create(m_pGraphicDev, vecGun[1]);
		NULL_CHECK_RETURN(pGameObject, E_FAIL);
...
HRESULT CStage::Ready_Layer_Trap(const _tchar * pLayerTag)
{
	...
	if (!vecThrone.empty())
	{
		for (int i = 0; i < vecThrone.size(); i++)
		{
			_tchar* szName = new _tchar[256]{};
